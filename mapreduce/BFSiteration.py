# run this file to obtain ONE iteration of BFS (does not automatically process the entire graph)
...
class MRBFSIteration(MRJob):
    ...
    def configure_args(self):
        ...
        # additional info that must be passed to every mapper node
        # see "self.options.target" in mapper()
    ...
    def reducer(self, key, values):
        ...
        for value in values:
            ...
            if (node.distance < distance):
                distance = node.distance # update the shortest distance between the node 
                # and the character we started from, using "node.distance = distance" later

            if ( node.color == 'BLACK' ):
                color = 'BLACK'

            if ( node.color == 'GRAY' and color == 'WHITE' ): # we have now seen the node, but have not epxlored who are the neighbouring nodes
                color = 'GRAY'
